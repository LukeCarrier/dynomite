...
static struct mbuf *get_mbuf(struct msg *msg) {
  struct mbuf *mbuf;
...
static void auth_reply(struct context *ctx, struct conn *conn, struct msg *smsg, const char *usr_msg) {
  int n;
  struct mbuf *mbuf;
...
static int get_password(unsigned char *string, unsigned int len, char *passwd, uint32_t *passwd_len) {
  char *p;
  char *pos;
  char buff[128];
  int cmdlen, nargc;
